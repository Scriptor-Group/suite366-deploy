...
discover_pg() {
  ...
  # `|| true`: grep exits 1 when nothing matches, and under `set -o pipefail`
  # that killed the caller before its own `|| die` could say so.
...
$(printf "${c_r}────────────────────────────────────────────────────────────────${c_0}")
  ...
  will be REPLACED:
    ...
    AI key     : "AIProvider".config->>'apiKey' (realigned onto THIS box's
                 llm/.env — the dump carries the key that was current when it
                 was taken, and no LLM call would work with it)
...
EOF
  ...
  # --- 1. extract ---------------------------------------------------------------
  chmod 0700 "$stage"
  log "1/8  Extracting snapshot '$snap'"
  ...
  # --- 2. reversibility ---------------------------------------------------------
  log "2/8  Snapshotting the CURRENT state first (tag pre-restore)"
  kc -n "$NAMESPACE" get secret "$app_secret" -o yaml \
  ...
  log "4/8  Carrying AUTH_SECRET over (before the data — see the header)"
  local k patched=0
  ...
  # --- 5. database ----------------------------------------------------------------
  log "5/8  Reloading the database"
  ...
  # --- 6. the key the restored database carries ---------------------------------
  # The dump holds the vLLM provider key that was current when it was TAKEN,
  # while values.yaml and llm/.env are explicitly NOT restored — so the row and
  # the containers now disagree, and every LLM call would 401. The app is still
  # at 0 replicas from step 3, so this lands before anything can read it.
  log "6/8  Realigning the stored vLLM provider key (the dump carries the old one)"
  reconcile_and_check_vllm_db 1

  # --- 6. objects -------------------------------------------------------------------
  if [[ -n "${minio_path:-}" && -d "$minio_path" ]]; then
    log "7/8  Restoring objects into $minio_path"
    local src
    ...
    log "7/8  No MinIO PVC resolved — skipping objects"
  fi

  # --- 7. back up, and prove it -----------------------------------------------------
  log "8/8  Restarting the app"
...
$(printf "${c_y}It is not done until you have verified it POSITIVELY:${c_0}")
  ...
  • make one LLM call — the stored provider key was realigned automatically in
    step 6/8, so a 401 here means something else and is worth reporting;
  • exercise one `enc:`-prefixed secret — an SSO sign-in, a bot integration or
    an agent connector. THAT is what proves AUTH_SECRET was carried over: the
    provider key above is stored in CLEAR ("AIProvider".config is plain jsonb),
    so an LLM call has never proven anything about it. A box that merely boots,
    and a user who merely logs in, prove nothing either.
